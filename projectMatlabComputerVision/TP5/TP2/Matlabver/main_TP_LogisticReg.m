...
for i=1:num_Im
    ...
    desc_Im_i = LBPfunction(ArrayIm{i},V);
    % Store the feature vector into the matrix X.
    X(i,:) = desc_Im_i;
end
DescripteurLBP=X;
save('DescripteurLBP','DescripteurLBP');
...
theta = glmfit(X_train,Y_train');
y_hat = fClassify_LogisticReg(X_test, theta');
Y_test_asig=(y_hat>0.5);
